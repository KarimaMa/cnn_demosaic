...
class DemosaicCNN(nn.Module):
    def __init__(self, k):
        ...
        self.conv3 = torch.nn.Conv2d(64, 60, (1,1), bias=False)
        self.relu3 = torch.nn.ReLU()
        self.conv4 = torch.nn.Conv2d(64, 8, (3,3), bias=False, padding=1)
    ...
    def forward(self, bayer):
        ...
        x = self.conv3(x)
        x = self.relu3(x)
        conv4in = torch.cat((bayer, x), dim=1)
        x = self.conv4(conv4in)
        x = self.relu4(x)
